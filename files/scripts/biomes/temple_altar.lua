...
function spawn_all_shopitems( x, y )
	...
	SetRandomSeed( x + newgame_n, y + newgame_n)
	...
	if( Random( 0, 100 ) <= 50 ) then
		for i=1,count do
			if( i == sale_item_i ) then
				generate_shop_item( x + (i-1)*item_width, y, true, biomeid, true )
			else
				generate_shop_item( x + (i-1)*item_width, y, false, biomeid, true )
			end
			
			generate_shop_item( x + (i-1)*item_width, y - 30, false, biomeid, true )
